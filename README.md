...
Needs testing/cleanup, and then design/code review. (This is a situation where "we don't know what we don't know", and thus the design is still a bit too "organic".) _**I need to meet with Alan/Jasmine/Jess about the test Web page design.**_

Soliciting feedback on Recommendations, below.

## synopsis

1. An [**RTCSignallingPeer** class](https://github.com/howard-stearns/web-connections/blob/master/public/rtc-signalling-peer.js#L261) wraps both RTCPeerConnection, _and_ signaling. 
 1. While the peer and its events are still available to applications, application-level methods are provided to add a data channel or a media stream. Different forms of signaling are handled by subclasses. (API below.)
 2. Internally, a **signalling protocol** is defined, with [implementations](https://github.com/howard-stearns/web-connections/blob/master/public/rtc-signalling-peer.js#L17) that use WebSockets (WS), or EventSource (SSE), or other mechanism for testing purposes. (This could be extended to, e.g., an overlay gossip network of peers, or Croquet, or other mechanisms, but there's no current need.)
 3. A small reference nodejs **signaling server** for WS and SSE of (1.2). The latter also has provisions for listing the currently available peers. ([source](https://github.com/howard-stearns/web-connections/blob/master/index.js))
2. **Testing**
 1. A [**unit test suite**](https://web-connections.herokuapp.com/SpecRunner.html) that exercises this between two peers in the same browser, through a matrix of (_specific-tests_ X _signaling-mechanism_). ([source](https://web-connections.herokuapp.com/browser-spec.js)) There are currently 21 specific tests, including various forms of "doing two things at once". There are four signaling-mechanisms tested: WS, SSE, direct "loopback" calls (because both peers are in the same browser), and loopback with a random delay for each message.
 2. A [**performance test page**](https://web-connections.herokuapp.com/) that tests for basic functionality, and then makes various performance tests of WS and SSE against the server and of RTCSignalingPeer against all other browsers that are still on that page. The performance tests repeat every 30 minutes. ("Credits" are given as an incentive to keep the page up, and as a crude test of whether people will contribute resources if it is easy enough.) 

Note: This is entirely Javascript. There is not yet a C++ implementation of RTCSignallingPeer.
...
- _**Zach has given me an additional set of features that he is using that I am not yet testing/reporting. I also need to get lists from Sam and Chris.**_
